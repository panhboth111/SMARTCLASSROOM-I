...
class UserService {
    async allUsers(role,owner){
        return new Promise((resolve,reject)=>{
            ...
            if(!adminReg.test(role)) reject({ message: "You are not authorized!", errCode: "AU-001" })
            User.find({ email: { $ne: owner }, role: { $ne: "Admin" } }, { "email": 1, "name": 1, "role": 1 }).then(users => resolve(users))
        })
            ...
            bcrypt.genSalt(10, async (err, salt) => {
                ...
                bcrypt.hash(pwd, salt, async (err, hash) => {
                    ...
                    try{
                        ...
                        if (err.code == 11000) resolve({"message" : "Email is already registered!","errCode" : "SU-002"})
    ...
    async signIn({email,pwd}){
        return new Promise(async (resolve,reject)=>{
            ...
            if (validateRes == undefined) resolve("invalid")
            const existUser = await Credential.findOne({email:email})
            if (!existUser) resolve("noEmail")
            const user = await User.findOne({email})
    ...
    async  changePassword(email,password,newPassword){
        return new Promise(async (resolve,reject)=>{
            if(password==newPassword) resolve({"message" : "New password can't be the same to the previous password!", errCode : "CP-001"})
            const constraint = {
                password : {
                    presence : true,
                    length : {
                        ...
                        bcrypt.hash(newPassword, salt, async (err, hash) => {
                            ...
                            try{
                                ...
                                if (result.n >= 1){
                                    ...
                                    resolve({message : "Problem Occured during the process of changing password!", errCode : "CP-003"})
                                }  
                            }catch(err){
    ...
    async changeRole({email,role},{targetUser,targetRole}){
        return new Promise(async (resolve,reject)=>{
            ...
            if (roleReg.test(role)) {
                const update = await User.updateOne({ email: targetUser }, { role: targetRole })
                if (update.n > 0) resolve({ "message": "Success fully update user '" + targetUser + "' to '" + targetRole + "'" })
                else resolve({ "messgage": "An error occurred during role changing process", "errCode": "CR-002" })
            } 
